...
public class JwtUtils {
    ...
    private static final long TOKEN_EXPIRE_MILLIS = 24 * 60 * 60 * 1000;
    /**
     * 生成 JWT Token
     *
     * @param userId 用户 ID过期时间（毫秒）
     * @return JWT 字符串
     */
    public static String sign(String userId) {
        try {
            ...
            JWTClaimsSet claimsSet = new JWTClaimsSet.Builder()
                    .subject(userId)
                    .issueTime(new Date())
                    .expirationTime(Date.from(Instant.now().plusMillis(TOKEN_EXPIRE_MILLIS)))
                    .build();
            // 2. 创建签名头
    ...
    public static String getLoginTokenKey(String userId){
        return "identity-service:token:" + userId;
    }
